...
static int ef10_mcdi_rpc_client(struct efhw_nic *nic, uint32_t client_id,
...
#if 0
static int ef10_fw_rpc(struct efhw_nic *nic, struct efx_auxdev_rpc *rpc)
{
  int rc;
  struct device *dev;
  struct efx_auxdev *auxdev;
...
static int
_ef10_ef100_nic_check_supported_filter(ci_dword_t* matches, int len,
                                       unsigned filter)
{
  int i;
  for(i = 0; i < len; i++)
...
int ef10_mcdi_cmd_event_queue_enable(struct efhw_nic *nic, uint32_t client_id,
                                     ...
                                     uint enable_rx_merging, uint enable_timer)
{
  ...
  rc = ef10_mcdi_rpc_client(nic, client_id, MC_CMD_INIT_EVQ, in_size,
                            sizeof(out), &out_size, in, &out);
/* FIXME SCJ
...
void ef10_mcdi_cmd_event_queue_disable(struct efhw_nic *nic,
                                       uint32_t client_id, uint evq)
{
  ...
  rc = ef10_mcdi_rpc_client(nic, client_id, MC_CMD_FINI_EVQ,
                            MC_CMD_FINI_EVQ_IN_LEN, 0, &out_size, in, NULL);
                            ...
                            struct efhw_evq_params *params)
{
  ...
  rc = ef10_mcdi_cmd_event_queue_enable(nic, client_id, params,
                                        enable_cut_through, enable_rx_merging,
                                        enable_timer);
  ...
  if( rc == 0 && enable_time_sync_events ) {
    rc = _ef10_mcdi_cmd_ptp_time_event_subscribe(nic, params->evq,
                                                 ...
                                                 __FUNCTION__);
  if( rc != 0 ) {
    ef10_ef100_mcdi_cmd_event_queue_disable(nic, client_id, params->evq);
                             ...
                             uint evq, int time_sync_events_enabled)
{
  ...
  ef10_mcdi_cmd_event_queue_disable(nic, client_id, evq);
}
...
static int
_ef10_mcdi_cmd_set_multicast_loopback_suppression(struct efhw_nic *nic,
 ...
 *---------------------------------------------------------------------------*/
int
ef10_mcdi_cmd_init_txq(struct efhw_nic *nic, uint32_t client_id,
                       ...
                       int flag_m2m_d2c)
{
  ...
  /* FIXME SCJ currently have net driver vport id, not mcdi vport id */
  EFHW_MCDI_SET_DWORD(in, INIT_TXQ_EXT_IN_PORT_ID,
                      EVB_STACK_ID(params->stack_id) | EVB_PORT_ID_ASSIGNED);

  EFHW_MCDI_POPULATE_DWORD_12(in, INIT_TXQ_EXT_IN_FLAGS,
              ...
              INIT_TXQ_EXT_IN_FLAG_M2M_D2C, flag_m2m_d2c);

  for( i = 0; i < params->n_dma_addrs; i++ )
    EFHW_MCDI_SET_ARRAY_QWORD(in, INIT_TXQ_EXT_IN_DMA_ADDR, i,
                              params->dma_addrs[i]);

  rc = ef10_mcdi_rpc_client(nic, client_id, MC_CMD_INIT_TXQ,
		    ...
		    struct efhw_dmaq_params *params)
{
  ...
  if (flag_loopback) {
    if (nic->flags & NIC_FLAG_MCAST_LOOP_HW) {
      rc = _ef10_mcdi_cmd_set_multicast_loopback_suppression(nic, 1,
                                                             params->vport_id,
                                                             params->stack_id);
      if (rc == -EPERM) {
        /* Few notes:
  ...
  for (flag_pacer_bypass = 1; 1; flag_pacer_bypass = 0) {
    rc = ef10_mcdi_cmd_init_txq(nic, client_id, params,
                                flag_timestamp, QUEUE_CRC_MODE_NONE,
                                flag_tcp_udp_only, flag_tcp_csum_dis,
                                flag_ip_csum_dis, flag_buff_mode,
                                flag_pacer_bypass, flag_ctpio,
                                flag_ctpio_uthresh, flag_m2m_d2c);
                    ...
                    struct efhw_dmaq_params *params)
{
  ...
  EFHW_MCDI_DECLARE_BUF(out, MC_CMD_INIT_RXQ_V4_OUT_LEN);
  EFHW_MCDI_INITIALISE_BUF(in);
  EFHW_MCDI_INITIALISE_BUF(out);
  ...
  EFHW_MCDI_SET_DWORD(in, INIT_RXQ_V4_IN_OWNER_ID,
                      REAL_OWNER_ID(params->owner));
  /* FIXME SCJ currently have net driver vport id, not mcdi vport id */
  EFHW_MCDI_SET_DWORD(in, INIT_RXQ_V4_IN_PORT_ID,
                      EVB_STACK_ID(params->stack_id) | EVB_PORT_ID_ASSIGNED);
  ...
  rc = ef10_mcdi_rpc_client(nic, client_id, MC_CMD_INIT_RXQ,
                            MC_CMD_INIT_RXQ_V4_IN_LEN,
                            MC_CMD_INIT_RXQ_V4_OUT_LEN, &outlen, in, out);
...
static int
_ef10_mcdi_cmd_fini_rxq(struct efhw_nic *nic, uint32_t client_id,
                        uint32_t instance)
{
  int rc;
  size_t out_size;
  ...
  rc = ef10_mcdi_rpc_client(nic, client_id, MC_CMD_FINI_RXQ,
                            MC_CMD_FINI_RXQ_IN_LEN, 0, &out_size, in, NULL);
...
static int
_ef10_mcdi_cmd_fini_txq(struct efhw_nic *nic, uint32_t client_id,
                        uint32_t instance)
{
  int rc;
  size_t out_size;
  ...
  rc = ef10_mcdi_rpc_client(nic, client_id, MC_CMD_FINI_TXQ,
                            MC_CMD_FINI_TXQ_IN_LEN, 0, &out_size, in, NULL);
...
int ef10_flush_tx_dma_channel(struct efhw_nic *nic, uint32_t client_id,
                              uint dmaq, uint evq)
{
  return _ef10_mcdi_cmd_fini_txq(nic, client_id, dmaq);
}


int ef10_flush_rx_dma_channel(struct efhw_nic *nic, uint32_t client_id,
                              uint dmaq)
{
  return _ef10_mcdi_cmd_fini_rxq(nic, client_id, dmaq);
}
               ...
               u32 efhw_rss_mode, int num_qs, u32 *rss_context_out)
{
  ...
  rc = ef10_rss_mode_to_nic_flags(nic, efhw_rss_mode, &nic_rss_flags);
  if (rc < 0)
  return rc;
...
static int
ef10_rss_update(struct efhw_nic *nic, const u32 *indir, const u8 *key,
                     ...
                     struct efx_filter_spec *spec)
{
  ...
  /* FIXME SCJ check param for rss context if not using one */
  unsigned rss_context = (spec->flags & EFX_FILTER_FLAG_RX_RSS ) ?
                          spec->rss_context : 0;
  int rc;

  /* FIXME SCJ based on dl, check how we actually want to handle reset */
  /* If [auxdev] is NULL, the hardware is morally absent and so there's
   * nothing to do. */
  if (auxdev == NULL) {
    put_device(dev);
